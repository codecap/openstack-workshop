...
{
cat <<EOF
$(basename $0) --id 10 --hostname myname [ARGs]
    --id       - server id
    --hostname - hostname
    --cpu      - number of CPUs
    --ram      - RAM (Gb)
...
echo qm clone $TEMPLATE_ID $VMID --name $NAME --full

echo qm set $VMID --ciuser deploy
...
while IFS=$'\t' read -r NET_NAME IP GW MTU; do
    ...
    echo qm set $VMID --net$NEXT_ID      "$NCARD_MODEL,bridge=br$BRIDGE"
    echo qm set $VMID --ipconfig$NEXT_ID "ip=${IP}${GW:+,gw=$GW}${MTU:+,mtu=$MTU}"
done < <(echo "$NETCONF_JSON" | jq -r 'to_entries[] | "\(.key)\t\(.value.ip // "")\t\(.value.gw // "")\t\(.value.mtu // "")"')


if [ ! -z "$NAMESERVER" ]
then
  echo qm set $VMID --nameserver "$NAMESERVER"
fi

if [ ! -z "$SEARCHDOMAIN" ]
then
  echo qm set $VMID --searchdomain "$SEARCHDOMAIN"
fi
...
while read SIZE; do
    ...
    echo qm set "$VMID" "--scsi$NEXT_ID" "${STORAGE}:${SIZE}"
